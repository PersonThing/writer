<script>
  ...
  async function handleDrop(e, path) {
    ...
    await project.reorderDone(draggedPath, path);
    draggedPath = null;
    dragOverPath = null;
  }
...
<div class="done-panel">
  <div class="done-hdr">
    ...
    <span class="done-count">{project.doneOrder.length || ''}</span>
  </div>
  <div class="done-list">
    {#if project.doneOrder.length === 0}
      <div class="done-empty">Mark poems as Done to build your reading order</div>
    {:else}
      {#each project.doneOrder as path, idx}
