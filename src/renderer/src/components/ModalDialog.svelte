...
{#if ui.modal}
  <div class="modal-overlay" onclick={handleOverlayClick} onkeydown={handleKeydown} role="dialog">
    <div class="modal-box">
      ...
      <div class="modal-actions">
        {#if ui.modal.type !== 'alert'}
          <button class="btn-small" onclick={handleCancel}>{ui.modal.cancelLabel}</button>
        {/if}
        <button class="btn-primary btn-small" onclick={handleConfirm}>{ui.modal.confirmLabel}</button>
      </div>
    </div>
  </div>
{/if}
...
<style>
  ...
  .modal-box {
    background: var(--surface); border-radius: 10px; padding: 1.4rem 1.6rem;
  ...
  .modal-input {
    width: 100%; font-size: .85rem; padding: .45rem .65rem;
